...
An AI-runtime update reports higher throughput, identical model outputs, and passing component checks. Under operating load, however, one decision arrives **180 ms late**. The conveyor has already moved the parcel; the gripper closes after the package passes the pickup point, and the release is blocked.

This deliberately tests an outcome independent of the update itself:

- component checks: build, API, model-output equivalence, throughput;
